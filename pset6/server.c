...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int countSpaces = 0;
            int hasPeriod = 0;
            for (int i = 0, s = strlen(line); i < (s - 2); i++)
            {
                if(isspace(line[i]))
                {
                    countSpaces++;
                }
                if(isspace(line[i]) && countSpaces == 1 && (i + 1) < s)
                {
                    if(line[i + 1] != '/')
                    {
                        error(501);
            ...
            for (int i = strlen(line) - 2, s = strlen(line); i < s; i++)
            {
                if(isspace(line[i]))
                {
                    countSpaces++;
            ...
            if(tolower(line[0]) != 'g' && tolower(line[1]) != 'e' && tolower(line[2]) != 't')
            {
                error(405);
            }
            if(strstr(line, "\"") != NULL)
            ...
            char query[strlen(line)];
            bool qmarkFound = false;
            int index = 0;
            
            if(strstr(line, "?") != NULL)
            {
                for(int i = 0, s = strlen(line) - 2; i < s; i++)
                {
                    if(qmarkFound && !isspace(line[i]))
                    {
                        query[index] = line[i];
                        index++;
                    }
            ...
            char path[strlen(root) + strlen(line)];
            int spaces = 0;
            bool flag = true;
            index = 0;
            
            for(int i = 0, s = strlen(root); i < s; i++)
            {
                path[i] = root[i];
                index++;
            }
            ...
            for(int i = 0, s = strlen(line) - 2; i < s; i++)
            {
                if(line[i] == '?' || (isspace(line[i]) && spaces > 0))
                {
                    flag = false;
                }
            ...
            flag = false;
            index = 0;
            for(int i = 0, s = strlen(path); i < s; i++)
            {
                if(flag)
                {
                    extension[index] = path[i];
                    index++;
                }
                if(path[i] == '.')
                {
...
const char* lookup(const char* extension)
{
    ...
    // TODO COMPLETED
    char converted[strlen(extension) + 1];
    strcpy(converted, extension);

    for(int i = 0, s = strlen(extension);  i < s; i++)
    {
        converted[i] = tolower(extension[i]);
    }
    
    // css
    if(strcmp(converted, "css") == 0)
    {
        return "text/css";
    }
    // html
    else if(strcmp(converted, "html") == 0)
    {
        return "text/html";
    }
    // gif
    else if(strcmp(converted, "gif") == 0)
    {
        return "image/gif";
    }
    // ico
    else if(strcmp(converted, "ico") == 0)
    {
        return "image/x-icon";
    }
    // jpg
    else if(strcmp(converted, "jpg") == 0)
    {
        return "image/jpeg";
    }
    // js
    else if(strcmp(converted, "js") == 0)
    {
        return "text/javascript";
    }
    // png
    else if(strcmp(converted, "png") == 0)
    {
        return "image/png";
    }
    else
